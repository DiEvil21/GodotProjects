...
onready var x_start = ((get_tree().get_root().get_size().x / 2.0) - ((width/2.0) * offset ) + (offset / 2))
onready var y_start = ((get_tree().get_root().get_size().y / 2.0) + ((height/2.0) * offset ) - (offset / 2))
